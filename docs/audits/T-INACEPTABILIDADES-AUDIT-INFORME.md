...
> **Origen** · ejecutado por CC siguiendo spec `docs/audits/T-INACEPTABILIDADES-AUDIT.md`
> **Fecha** · 2026-05-09
...
$ grep -rnE "deleteMovement|removeMovement|editMovement|updateMovement|reclasifyMovement" src/ --include="*.ts" --include="*.tsx"
...
deduplicationService → enhancedDeduplicationService
```
...
`ls -la src/modules/v5/` · vacío salvo subdirectorios `ajustes archivo financiacion fiscal horizon inmuebles inversiones mi-plan onboarding panel personal pulse shared tesoreria`. Anomalía detectada · `src/modules/v5/horizon/` debería NO existir si v5 reemplaza horizon. NO se inspeccionó en profundidad por estar fuera del scope T-INACEPTABILIDADES.
...
- A3 AEAT · 5 servicios + 3 wizards anidados (uno envuelve a otros 2) · el wrapper v5 `ImportarFiscalPage.tsx` sólo importa el legacy horizon · construcción tipo "muñeca rusa" · probablemente la mitad de los 5 servicios son granularidad legítima (XML vs PDF vs Planes Pensión) pero `aeatParserService` sin sufijo es sospechoso de ser fachada o duplicidad.
...
| C5 | **Restore desde snapshots de declaración (`snapshotsDeclaracion`)** | Parcial | Store dedicado en IndexedDB · pero NO función `loadSnapshotDeclaracion()`/`restoreFromSnapshot()` · sólo lectura para visualización histórica | Viva como almacén · NO como restore | — · es snapshot fiscal "freezed" para auditoría · no backup-restore | Sin dead code. |
