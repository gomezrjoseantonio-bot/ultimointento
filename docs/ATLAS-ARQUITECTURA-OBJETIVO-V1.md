...
Recuperación de cache: `failed` debe reintentarse automáticamente con backoff corto y contador de intentos; tras N fallos queda en `dirty_failed` y requiere acción manual “Recalcular saldos”. Mientras tanto, la UI usa cálculo on-demand y muestra aviso de cache no sincronizada. Si llega otro `markDirty` durante `recomputing`, se acumula en una cola por `accountId` y al terminar se ejecuta un segundo recompute antes de volver a `clean`.
...
8. Recuperación ante fallo:
   - **XML fallido**: cada ejercicio se importa como batch idempotente. La frontera transaccional recomendada es aplicación/batch, no una transacción IndexedDB gigante. Cada registro escrito guarda `importBatchId`, `ejercicio` y `sourceHash`; si falla un ejercicio, una compensación borra todos los registros de ese batch/ejercicio.
   - **Fallo multi-año**: si 2020-2022 terminaron y falla 2023, se conservan 2020-2022 como batches `completed` y se revierte solo 2023. Si Jose exige atomicidad total, alternativa: reiniciar desde DB vacía usando el snapshot pre-wipe.
   - **CSV fallido**: se borra el `importBatch` parcial y sus `movements` asociados antes de reintentar.
   - **Fase posterior fallida**: se puede repetir desde DB vacía con los mismos XML/CSV o restaurar el snapshot pre-wipe.
9. Mensajes operativos al usuario:
   - XML fallido → “No se importó el ejercicio YYYY; no se ha escrito ningún dato parcial de ese ejercicio. Corrige el XML o reintenta.”
   - CSV fallido → “El extracto no se completó; se ha eliminado el lote parcial y puedes volver a subirlo.”
   - Reconciliación fallida → “Los datos importados siguen guardados; revisa las partidas marcadas y reintenta la conciliación.”
   - Fallo crítico post-wipe → “Restaurar snapshot previo” debe estar disponible como acción visible.
