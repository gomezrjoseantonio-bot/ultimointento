...
8. Recuperación ante fallo: cada ejercicio XML debe importarse dentro de un batch lógico idempotente. La frontera transaccional recomendada es aplicación/batch, no una única transacción IndexedDB gigante: cada store escrito guarda `importBatchId`, `ejercicio` y `sourceHash`; si falla un ejercicio, una compensación borra todos los registros de ese batch/ejercicio. Si 2020-2022 ya terminaron y falla 2023, se conservan 2020-2022 como batches `completed` y se revierte solo 2023; si Jose prefiere atomicidad total, el plan alternativo es reiniciar desde DB vacía usando el snapshot pre-wipe. Si falla la importación bancaria, se borra el `importBatch` parcial y sus `movements` asociados antes de reintentar. Si falla una fase posterior, se puede repetir desde DB vacía usando los mismos XML/CSV o restaurar el snapshot pre-wipe.
