...
export async function ejecutarMigracion(): Promise<void> {
  ...
  for (const summary of summaries) {
    for (const { campo, casilla, categoria } of CASILLAS) {
      try {
        ...
        errores1++;
        console.warn(`[migracion] Error migrando fiscalSummary ${summary.propertyId}-${summary.exerciseYear}:`, err);
      }
    }
  }
  if (errores1 > 0) console.warn(`[migracion] Bloque 1 (fiscalSummaries): ${errores1} registros con error — ignorados`);
  ...
  if (db.objectStoreNames.contains('mejorasActivo')) {
    ...
    for (const m of mejoras) {
      try {
        // Dedup: comprobar si ya existe por inmuebleId + ejercicio + descripcion + importe
        const existentes = await db.getAllFromIndex('mejorasInmueble', 'inmueble-ejercicio', [m.inmuebleId, m.ejercicio]);
        const yaMigrado = existentes.some(e => e.descripcion === (m.descripcion || '') && Math.abs(e.importe - (m.importe || 0)) < 0.01);
        ...
        errores2++;
        console.warn(`[migracion] Error migrando mejoraActivo ${(m as any).id}:`, err);
      }
    }
  }
  if (errores2 > 0) console.warn(`[migracion] Bloque 2 (mejorasActivo): ${errores2} registros con error — ignorados`);
  ...
  if (db.objectStoreNames.contains('mobiliarioActivo')) {
    ...
    for (const m of muebles) {
      try {
        // Dedup: comprobar si ya existe por inmuebleId + ejercicio + descripcion + fechaAlta
        const ejercicio = m.ejercicio || new Date(m.fechaAlta).getFullYear() || new Date().getFullYear();
        const existentes = await db.getAllFromIndex('mueblesInmueble', 'inmueble-ejercicio', [m.inmuebleId, ejercicio]);
        const yaMigrado = existentes.some(e => e.descripcion === (m.descripcion || '') && e.fechaAlta === m.fechaAlta);
        if (yaMigrado) continue;
        ...
        errores3++;
        console.warn(`[migracion] Error migrando mobiliarioActivo ${(m as any).id}:`, err);
      }
    }
  }
  if (errores3 > 0) console.warn(`[migracion] Bloque 3 (mobiliarioActivo): ${errores3} registros con error — ignorados`);
  ...
  if (db.objectStoreNames.contains('operacionesFiscales')) {
    ...
    for (const op of operaciones) {
      try {
        ...
        errores4++;
        console.warn(`[migracion] Error migrando operacionFiscal ${op.id}:`, err);
      }
    }
  }
  if (errores4 > 0) console.warn(`[migracion] Bloque 4 (operacionesFiscales): ${errores4} registros con error — ignorados`);
