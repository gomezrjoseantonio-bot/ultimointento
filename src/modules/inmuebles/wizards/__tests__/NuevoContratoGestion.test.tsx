...
import NuevoContratoGestion from '../NuevoContratoGestion';
...
const renderForm = () =>
  render(
    <MemoryRouter initialEntries={['/contratos/gestion/nuevo']}>
      <Routes>
        <Route element={<Outlet context={ctx} />}>
          <Route path="/contratos/gestion/nuevo" element={<NuevoContratoGestion />} />
        </Route>
...
test('happy path · guarda agencia + contrato de gestión con bloque gestion y navega', async () => {
  ...
  // Espera a que la cuenta cargue y aparezca su opción antes de seleccionarla.
  ...
  // Cuenta de cobro · último combobox.
  fireEvent.change(combos[combos.length - 1], { target: { value: '7' } });
