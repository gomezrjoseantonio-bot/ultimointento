import { Ingreso, Gasto, AEATFiscalType } from './db';

/** Local MejoraRecord type for validation (mejorasInmueble store) */
type MejoraRecord = {
  id?: number;
  inmueble_id: number;
  contraparte: string;
 ...
 * 
 * Provides validation logic for Treasury records (Ingresos, Gastos, Mejoras)
...
export const validateGasto = (gasto: Partial<Gasto>): ValidationResult => {
  ...
  if (gasto.categoria_AEAT === 'capex-mejora-ampliacion') {
    warnings.push('Esta categoría AEAT debería usarse para mejoras, no gastos regulares');
  }

  if (gasto.categoria_AEAT === 'amortizacion-muebles') {
    warnings.push('Esta categoría AEAT debería usarse para mejoras de mobiliario, no gastos regulares');
  }
...
// Mejora Validation
export const validateMejora = (mejora: Partial<MejoraRecord>): ValidationResult => {
  const errors: string[] = [];
  const warnings: string[] = [];

  // Required fields validation
  if (!mejora.inmueble_id) {
    errors.push('El inmueble es obligatorio');
  }

  if (!mejora.contraparte?.trim()) {
    errors.push('El proveedor es obligatorio');
  }

  if (!mejora.fecha_emision) {
    errors.push('La fecha de emisión es obligatoria');
  }

  if (!mejora.total || mejora.total <= 0) {
    errors.push('El total debe ser mayor que 0');
  }

  if (!mejora.tipo) {
    errors.push('El tipo de mejora es obligatorio');
  }

  if (!mejora.anos_amortizacion || mejora.anos_amortizacion <= 0) {
    errors.push('Los años de amortización deben ser mayor que 0');
  }

  // Business logic validation
  if (mejora.anos_amortizacion) {
    if (mejora.tipo === 'mobiliario' && mejora.anos_amortizacion !== 10) {
      warnings.push('El mobiliario típicamente se amortiza en 10 años según normativa AEAT');
    }

    if (mejora.tipo === 'mejora' && (mejora.anos_amortizacion < 10 || mejora.anos_amortizacion > 50)) {
      warnings.push('Las mejoras típicamente se amortizan entre 10 y 50 años');
    }

    if (mejora.tipo === 'ampliacion' && (mejora.anos_amortizacion < 15 || mejora.anos_amortizacion > 50)) {
  ...
  // Date validation
  if (mejora.fecha_emision) {
    const emisionDate = new Date(mejora.fecha_emision);
    const today = new Date();
    const oneYearAgo = new Date(today.getTime() - 365 * 24 * 60 * 60 * 1000);
    const oneYearFromNow = new Date(today.getTime() + 365 * 24 * 60 * 60 * 1000);
  ...
  if (mejora.total) {
    if (mejora.total < 100) {
      warnings.push('El importe es muy bajo para ser considerado mejora (menos de 100€)');
    }

    if (mejora.total > 200000) {
...
export const validateTreasuryBatch = (records: {
  ...
  mejoras?: Partial<MejoraRecord>[];
}): {
  ingresos: ValidationResult[];
  gastos: ValidationResult[];
  mejoras: ValidationResult[];
  summary: {
    totalRecords: number;
    validRecords: number;
  ...
  const mejoraResults = (records.mejoras || []).map(validateMejora);

  const allResults = [...ingresoResults, ...gastoResults, ...mejoraResults];
