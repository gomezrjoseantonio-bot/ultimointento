  ...
  if (!parseResult) {
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
          <div className="text-center">
            <X className="w-8 h-8 text-error-500 mx-auto mb-4" />
          ...
          <div className="flex items-center gap-3">
            <div>
              ...
              <p className="text-sm text-neutral-600 mt-1">
                ...
                {parseResult.metadata?.confidence && (
                  <span className="ml-2 px-2 py-1 bg-success-100 text-success-700 rounded text-xs">
            ...
            {parseResult.success && parseResult.metadata?.bankName && parseResult.metadata.bankKey !== 'manual' && (
              <div className="flex items-center gap-2 px-3 py-2 bg-success-50 border border-success-200 rounded-lg">
                ...
                <span className="text-sm text-success-800">Detección automática</span>
              </div>
            )}
            ...
            {parseResult.sheetInfo && parseResult.sheetInfo.length > 1 && (
              <div className="flex items-center gap-2 px-3 py-2 bg-primary-50 border border-primary-200 rounded-lg">
                ...
                <span className="text-sm text-primary-800">Multi-hoja</span>
              </div>
            )}
          </div>
          <button
            ...
            {distributions.map((distribution, index) => (
              <div
                ...
                className={`border rounded-lg p-4 ${
                  selectedMovements.includes(index) ? 'border-primary-300 bg-primary-50' : 'border-neutral-200'
                }`}
              >
                <div className="flex items-center gap-4">
                  <input
                  ...
                  <div className="flex-1 grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div>
                      <div className={`text-sm font-medium ${
                        distribution.movement.amount >= 0 ? 'text-success-600' : 'text-error-600'
                      }`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="px-3 py-2 bg-success-600 text-white rounded text-sm hover:bg-success-700"
                >
                  Crear
                </button>
                <button
