...
// ============================================================================
//
// Prefijo `__` indica herramienta de desarrollo: no la llama ninguna pantalla,
// no pinta nada y no escribe. Su único consumidor es la consola del navegador,
// donde se cuelga de `window.atlasDiagnostico`.
//
// Sí se REGISTRA desde producción (`TesoreriaV6Page`), igual que
// `duplicadosPrevisionService`, y a propósito: las páginas `/dev/*` están
// apagadas en producción (`REACT_APP_ENABLE_DEV_PAGES`) y los datos que hay que
// mirar están justamente en el navegador de producción. Una herramienta de
// diagnóstico que no se puede ejecutar donde está el problema no sirve de nada.
// Lo que NO debe hacer nadie es importarla desde lógica de negocio.
//
...
// La auditoría de líneas fiscales vive en su propio módulo: mira otro store y
// contesta otra pregunta.
import { lineasFiscalesAnomalas } from './__lineasFiscalesAudit';
import type { ResumenFiscal } from './__lineasFiscalesAudit';

// Se reexporta para no romper a quien ya la importaba de aquí.
export { lineasFiscalesAnomalas } from './__lineasFiscalesAudit';
export type { AnomaliaFiscal, LineaFiscalAnomala, ResumenFiscal } from './__lineasFiscalesAudit';
