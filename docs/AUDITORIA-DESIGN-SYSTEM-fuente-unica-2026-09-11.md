...
5. **Regla real de importes (Tesorería, vinculante)**: los números van **siempre en tinta**; el **oro** solo para la cifra-veredicto; el **ámbar** solo donde hay que actuar. **Verde/rojo nunca colorean un importe en Tesorería**; solo marcan estado (cuadre OK / descuadre / error de validación). Otros módulos (Panel, Inversiones, Personal, Inmuebles) sí colorean deltas, rentabilidades y saldos pendientes por signo: hay que decidir si eso se mantiene (§3.4). Además, los totales de la tabla de cuentas usan `gold-ink` (`TablaBanco.module.css:112`), que la regla prohíbe en cifras: incumplimiento pendiente (§3.2).
...
Recuento global (ts, tsx, css): `grep -rl 'atlas-v5-' src` → **271 ficheros**; `grep -rn 'atlas-v5-' src | wc -l` → **10.209 líneas**.

### 1.3 · Dónde vive la guía · aviso

La guía V5 está en `docs/audit-inputs/`, carpeta que su propio README describe como inputs de la "TAREA 6 · arquitectura de stores" (`docs/audit-inputs/README.md:1-9`). No es sitio para el documento normativo del design system. Propuesta post-OK: mover a `docs/GUIA-DISENO-V5-atlas.md`. Es solo un cambio de ruta, pero la ruta la citan **34 ficheros** (`grep -rl GUIA-DISENO-V5-atlas`: `tokens.css:4`, handoffs, 4 specs en `docs/specs/`, 4 en `docs/audit-inputs/`, 3 en `src/modules/`, tareas y auditorías). Hay que actualizar todas en el mismo PR o dejar un enlace de redirección en la ruta vieja.
...
| Q | Scripts v3 · `scripts/atlas-lint.js` (`npm run lint:atlas`, `build:atlas`), `scripts/migrate-to-atlas.js`, `scripts/migrate-buttons-to-atlas.js` (`migrate:atlas`, `migrate:buttons`), `scripts/replace-hardcoded-colors.js` (`migrate:colors`) | 51 + 273 + lint + reemplazador | **FÓSIL** | El lint prohíbe Arial/Helvetica/Times/Google Fonts (`atlas-lint.js:66-73`) tolerando Inter/system-ui (`:284-287`) y lo llama "ATLAS v3 requires IBM Plex" (`:65`, `:296`); prohíbe `#09182E` (`:44`) y remite a `/design-bible/` (`:461`). Los migradores reescriben Tailwind a `text-ok`, `text-error`, `text-atlas-blue` (`migrate-to-atlas.js:6-11`) y `replace-hardcoded-colors.js:14-75` reescribe hex a `var(--atlas-blue)`, `var(--ok)`, `var(--warn)`, `var(--error)` (34 mapeos): los nombres VIEJOS. Ninguno corre en CI (`.github/workflows` no los invoca) | **Borrar** los tres migradores y sus scripts npm. `atlas-lint.js`: borrar o reescribir contra V5 (decisión Jose) |
...
| …y se pintan con clases de tinta u oro: `.fuerte` (tinta) · `.oro` (oro) · `.mudo` (gris) | `ListaTarjetas.tsx:207,222,298` · `TablaBanco.module.css:104-114` |
| **Incumplimiento pendiente**: `.oro` (totales de cuentas y tarjetas) usa `gold-ink`, que la propia regla prohíbe en cifras ("se lee marrón"). La versión móvil sí usa `gold` | `TablaBanco.module.css:111-114` vs `TesoreriaV6Page.module.css:7` y `docs/TAREA-CC-TESORERIA-V5.md:271` · correcto en `TesoreriaMovil.module.css:84` |
...
| Inmuebles | mayoritariamente `tone="ink"` (28), pero el saldo pendiente del bote va en rojo/verde por signo y la diferencia de conciliación en `pos/neg/warn` | `contratos/DrawerConciliarBote.tsx:223,245-247,326` · `contratos/TabPorConciliar.tsx:120` |
| Todos | **83 usos de `MoneyValue` sin `tone`** heredan el `auto` (colorean por signo): `ResumenCockpitInmueble.tsx` (17), `LibertadPage.tsx` (10), `PatrimonioResumenInmueble.tsx` (9), `FiscalidadInmueble.tsx` (9), `personal/pages/PanelPage.tsx` (6), `CostesInmueble.tsx` (6), `EjerciciosPage.tsx` (5), etc. | `grep -rzoP '<MoneyValue[^>]*>' src \| grep -vc 'tone='` → 83 |
...
- **Opción 1 (recomendada · cuesta menos y respeta lo construido):** dejar la regla en dos niveles y escribirla explícitamente en la guía §2.2: *importes y saldos en tinta/oro (regla Tesorería); verde/rojo únicamente para deltas de rentabilidad (Inversiones, Panel) y para estados*. Cambio de código necesario: que `MoneyValue` deje de colorear por defecto (`tone` por defecto `'ink'` en vez de `'auto'`, `MoneyValue.tsx:42`), para que un mockup nuevo no herede el semáforo sin pedirlo. Ojo: los 7 usos con `tone="auto"` explícito no cambian, pero los **83 usos sin `tone`** (§3.3) pasarían a tinta; antes de cambiar el defecto hay que revisarlos uno a uno y poner `tone="auto"` explícito donde el color por signo sea intencional (Libertad, Personal, cockpit de inmueble). Es la parte laboriosa de la opción 1.
- **Opción 2 (más estricta):** verde/rojo nunca sobre un número, en ningún módulo. Implica repintar Panel, Inversiones, Personal, Mi Plan, Fiscal e Inmuebles (bote y conciliación de contratos), más los 83 usos sin `tone` (líneas de §3.3). Es migración → regla A, tarea aparte.

### 3.5 · Fondo amarillo

No existe. Tokens con tinte cálido: `--atlas-v5-gold-wash #F3EAD6`, `--atlas-v5-gold-wash-2 #FAF3E1`, `--atlas-v5-warn-wash #F5ECD6` (`tokens.css:46-47, 91`). Ninguno es amarillo: son beige/oro muy claro. Lo que la guía prohíbe como fondo de card es el `brand-wash` saturado (`GUIA V5:52`); los `gold-wash` sí se usan como superficie de cards oro en Inmuebles (`GananciaAcumuladaCard.module.css:7`, `ResumenCockpitInmueble.module.css:329-331`; 75 ficheros usan `gold-wash` en `background`). El "fondo amarillo" de los mockups fallidos no tiene token al que apuntar → es bug por definición (`GUIA V5:39`, regla 1).
...
Grep `#RRGGBB` en css/tsx/ts, excluyendo `tokens.css`, `bancoColores` y tests (`entidadLogo.ts` se muestra aparte porque son marcas):
...
| `src/modules/inmuebles/` | 6 | `components/contratos/historico/PanelAnalitico.module.css:136-140` (rampa de mapa de calor): `#F3EAD6` (`:137`) duplica el literal de `--atlas-v5-gold-wash` (`tokens.css:46`) en vez de usar el token; los otros cinco (`#EAF1EB #EFD9A8 #E8BE84 #DC9A7A #5C2718`) **no tienen token** → violación real |
...
| `src/index.css` | 9 hex + 5 rgba | `:21-22` teal, `:37` white, `:72-77` paleta chart v4 (`#042C5E`…), `:85` y `:382` focus ring rgba(4,44,94), `:170-171` sombras rgba(26,35,50) → **repuntar a tokens V5** (§7.2) |
...
#042C5E (--atlas-blue viejo)    → 4 líneas / 4 ficheros · index.css:72 (--c1 chart), ProjectionChart.tsx:25, AnalisisCartera.tsx:83, ImportarDeclaracionWizard.tsx:27 · pantallas legacy, NO como --atlas-blue
...
| 9 | `scripts/migrate-to-atlas.js`, `scripts/migrate-buttons-to-atlas.js`, `scripts/replace-hardcoded-colors.js` + `package.json:68,69,72` (`migrate:atlas`, `migrate:colors`, `migrate:buttons`) | Reescriben hacia nombres v3 (`text-ok`, `text-error`, `text-atlas-blue`, `var(--atlas-blue)`, `var(--ok)`, `var(--error)`) · no corren en CI. Dejar cualquiera de ellos permite reintroducir alias v3 |
| 10 | `scripts/atlas-lint.js` + `package.json:65-66` (`lint:atlas`, `build:atlas`) | Lint de la guía v3: prohíbe fuentes ajenas (`:66-73`), permite Inter/system-ui (`:284-287`), remite a `/design-bible/` (`:461`). No corre en CI. **Decisión Jose**: borrar o reescribir contra V5 (cero hex fuera de `tokens.css`) |
...
grep -rnE 'var\(--(navy|grey|blue|n-|hz-|atlas-blue)' src/modules/tesoreria/v6 --include=*.css  → 0 (recursivo · incluye v6/conciliar/)
grep -rn 'atlas-v5-pos' src/modules/tesoreria/v6 → 7 líneas (incluye pos-wash) · todas estado (CuadreBanco:18-20 · PanelConciliar:322,336-337 · DrawerV6:200)
grep -rn 'atlas-v5-neg' src/modules/tesoreria/v6 → 15 líneas (incluye neg-wash) · todas alerta/validación (DrawerExtracto:92,240,243 · FichaMovimiento:150,153,206 · PanelConciliar:166-171,456-457,497-499)
```

### 7.2 · Hex y rgba propios de la capa de alias (a repuntar, no a borrar)
...
src/index.css:85  --focus-ring: rgba(4,44,94,.12) → OJO: --atlas-v5-focus-ring-neutral es un box-shadow completo (tokens.css:242), no un color,
                  y --focus-ring se interpola dentro de `box-shadow: 0 0 0 4px var(--focus-ring)` (index.css:472,479) y en
                  ImageDescriptionComponent.tsx:224-255. Sustituir el valor rompería el foco. Opciones: (a) reemplazar las
                  declaraciones completas por `box-shadow: var(--atlas-v5-focus-ring-neutral)`, o (b) añadir en tokens.css un
                  token de COLOR de anillo (p. ej. --atlas-v5-focus-color) y apuntar --focus-ring a él.
src/index.css:382 box-shadow: 0 0 0 4px var(--focus-ring) · mismo caso
src/index.css:170-171 --shadow-1 / --shadow-2 rgba(26,35,50) → var(--atlas-v5-shadow-card) / var(--atlas-v5-shadow-lift)
...
Esos nombres existen solo como **colores Tailwind** (`tailwind.config.js:40-51`), no como variables CSS. El navegador descarta el valor → esos toasts salen sin color de borde/icono. Solo `toastService.tsx` usa esos seis nombres (`confirmationService.tsx` usa `--warn`, `--error`, `--atlas-blue`, que sí existen); a `toastService` lo importan 2 ficheros. Arreglo trivial post-OK: apuntar a `--atlas-v5-*`.
...
3. Borrar `treasury-v4.css`, `push-treasury-v3.sh`, `MovementStatusChip.tsx` + su test, `tax-view.css`, `ejercicio-selector.css`, `fiscal-tokens.css` (+ import), los tres scripts de migración v3 (`migrate-to-atlas`, `migrate-buttons-to-atlas`, `replace-hardcoded-colors`) + sus scripts npm. `atlas-lint.js` según decisión.
4. Cabecera en `src/index.css` y `tailwind.config.js`: "CAPA DE COMPATIBILIDAD · no es fuente de verdad · todo apunta a `src/design-system/v5/tokens.css`". Repuntar los 9 hex y 5 rgba (§7.2, con cuidado con `--focus-ring`). Arreglar `toastService.tsx:20-25` (§7.3).
5. Mover `GUIA-DISENO-V5-atlas.md` a `docs/`; actualizar las 34 referencias a la ruta (§1.3). Añadir en §2.2 la **regla de importes** (§3.4, opción elegida) y sincronizar §2.1 con `tokens.css` (§8.3).
6. `MoneyValue`: `tone` por defecto `'ink'` si se elige la opción 1 de §3.4, tras revisar los 83 usos sin `tone` y marcar `tone="auto"` explícito donde el color por signo sea intencional. Corregir `.oro` de `TablaBanco.module.css:112` a `--atlas-v5-gold` (§3.2).
