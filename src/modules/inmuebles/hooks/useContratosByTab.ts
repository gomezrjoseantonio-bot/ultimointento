...
export function useContratosByTab(tab: TabKey, contracts: Contract[]): Contract[] {
  return useMemo(() => {
    switch (tab) {
      case 'vigentes':
        return contracts.filter((c) => getEstadoEfectivo(c) === 'vigente');
      case 'proximos':
        return contracts.filter((c) => getEstadoEfectivo(c) === 'proximo');
      case 'historico':
        return contracts.filter((c) => getEstadoEfectivo(c) === 'finalizado');
      case 'disponibilidad':
      case 'analisis':
      case 'conciliar':
      default:
