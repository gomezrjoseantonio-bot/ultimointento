...
| Stores creados en `upgrade()` callback (runtime) | **40 stores activos** · incluye los 3 nuevos V65 (`planesPensiones`, `aportacionesPlan`, `traspasosPlanPensiones`) |
| Stores en interface `AtlasHorizonDB` (TS typing) | **40 entradas** · 2 legacy compile-only (`traspasosPlanes` y `objetivos_financieros`) + 38 vivos · los 3 nuevos V65 NO declarados como campos tipados (ver §1.1) |
| Discrepancia vs handoff "DB_VERSION 65 · 40 stores" | **NO** · cifra coincide |
...
### §1.1 · Nota técnica sobre tipado de stores V65

Los 3 stores nuevos V65 (`planesPensiones`, `aportacionesPlan`, `traspasosPlanPensiones`) **existen y funcionan en runtime** — se crean en el `upgrade()` callback (lines 2632-2660 y 3754-3923) y los servicios los usan vía `db.add('planesPensiones' as any, ...)`, `db.getAll('aportacionesPlan' as any)`, etc. (ver `src/services/aportacionesPlanService.ts`, `traspasosPlanPensionesService.ts`, `planesInversionService.ts`).

Lo único pendiente es **declararlos como campos en la interface TS `AtlasHorizonDB`** para retirar los `as any` casts. Es deuda cosmética de tipado, no funcional.

Adicionalmente la interface aún declara `traspasosPlanes` (line 2091) y `objetivos_financieros` (line 2203) como campos vivos pese a estar marcados como eliminados/migrados — son relictos de compile que conviene limpiar al mismo tiempo.
...
| Schema TS `AtlasHorizonDB` no tipa los 3 stores V65 nuevos (existen y funcionan en runtime, sólo falta tipar) | 🟢 bajo · deuda cosmética de tipado |
