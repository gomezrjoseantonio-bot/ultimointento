...
describe('ejercicioFiscalService', () => {
  ...
  test('Wrappers legacy: saveLegacyEjercicioRecord persiste la forma db', async () => {
    const saved = await saveLegacyEjercicioRecord({
      año: EJERCICIO,
      estado: 'vivo',
      origen: 'calculado',
      declaracionAeatOrigen: 'no_presentada',
    ...
    expect(saved.ejercicio).toBe(EJERCICIO);
    expect(saved.año).toBe(EJERCICIO);
    expect(saved.arrastresRecibidos?.porAnio).toEqual([]);
  });
