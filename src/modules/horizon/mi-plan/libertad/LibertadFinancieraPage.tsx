...
export const calcularPlanLibertad = ({
  ingresoObjetivo,
  cashflowActual,
  ahorroMensualNomina,
  capitalInicial,
...
export const calcularMesesSinBolaNieve = ({
  ingresoObjetivo,
  cashflowActual,
  ahorroMensualNomina,
  capitalInicial,
...
const LibertadFinancieraPage: React.FC = () => {
  ...
  const [metaIngresoMensual, setMetaIngresoMensual] = useState(3000);
  const [objetivoGuardado, setObjetivoGuardado] = useState(3000);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        setFinancialData({
          ingresoAlquilerNeto: toFiniteNumber(flujos.inmuebles.cashflow),
          ahorroMensualTotal:
            toFiniteNumber(flujos.trabajo.netoMensual) +
            toFiniteNumber(flujos.inmuebles.cashflow) +
            toFiniteNumber(flujos.inversiones.rendimientoMes) +
  ...
  const mesesSinBolaNieve = useMemo(() => calcularMesesSinBolaNieve(simulationConfig), [simulationConfig]);
  ...
  const guardarObjetivo = async () => {
    try {
      ...
      console.error('No se pudo guardar el objetivo', error);
            ...
            <>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mt-6">
                <label className="rounded-lg border p-4 block" style={{ borderColor: 'var(--n-200)' }}>
                  ...
                  />
                  {metaIngresoMensual !== objetivoGuardado && (
                    <button
                      type="button"
...
export default LibertadFinancieraPage;
