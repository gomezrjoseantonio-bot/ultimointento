...
// Legacy function to get rent calendar entries
export const getRentCalendar = async (contractId: number) => {
  const db = await initDB();
  try {
    // Try to get from the rentCalendar store if it exists
    const tx = db.transaction('rentCalendar', 'readonly');
    const store = tx.objectStore('rentCalendar');
    const index = store.index('contractId');
    const entries = await index.getAll(contractId);
    return entries;
  } catch (error) {
    // If rentCalendar store doesn't exist, return empty array
    console.warn('rentCalendar store not found, returning empty array');
    return [];
  }
};

// Legacy function to get rent payments
export const getRentPayments = async (contractId: number) => {
  const db = await initDB();
  try {
    // Try to get from the rentPayments store if it exists
    const tx = db.transaction('rentPayments', 'readonly');
    const store = tx.objectStore('rentPayments');
    const index = store.index('contractId');
    const entries = await index.getAll(contractId);
    return entries;
  } catch (error) {
    // If rentPayments store doesn't exist, return empty array
    console.warn('rentPayments store not found, returning empty array');
    return [];
  }
};

// Legacy function to mark payment as paid
export const markPaymentAsPaid = async (id: number, paidAmount: number, paymentDate: string, notes?: string) => {
  ...
  try {
    ...
    console.warn('markPaymentAsPaid: Legacy function called, store not available');
  }
};

// Legacy function to mark payment as partial
export const markPaymentAsPartial = async (id: number, paidAmount: number, paymentDate: string, notes?: string) => {
  ...
  try {
    ...
    console.warn('markPaymentAsPartial: Legacy function called, store not available');
  }
};

// Legacy alias for backward compatibility
export const terminateContract = rescindContract;
