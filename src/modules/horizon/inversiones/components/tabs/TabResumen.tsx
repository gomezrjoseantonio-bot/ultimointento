  ...
  return (
    <div>
      ...
      {planesPension.length > 0 && (
        ...
        >
          {(() => {
            const totalPensionAportado = planesPension.reduce(
              (sum, p) => sum + (p.aportacionesRealizadas || 0),
              0
            );
            const totalPensionValor = planesPension.reduce(
              (sum, p) =>
                sum +
                ((p.unidades
                  ? p.unidades * p.valorActual
                  : p.valorActual) || 0),
              0
            );
            ...
            return [
              {
                label: 'Planes de pensión acum.',
                val: formatCurrency(totalPensionAportado),
                meta: `${planesPension.length} plan${
                  planesPension.length > 1 ? 'es' : ''
                }`,
              },
              {
                label: 'Valor actual pensiones',
                val:
                  totalPensionValor > 0
                    ? formatCurrency(totalPensionValor)
                    : 'Sin actualizar',
                meta: 'Último valor conocido',
              },
              {
                label: 'Plusvalía / Minusvalía',
                val: `${pensionGP >= 0 ? '+' : ''}${formatCurrency(pensionGP)}`,
                meta: 'Sobre aportaciones realizadas',
              },
            ];
          })().map((k) => (
            <div
