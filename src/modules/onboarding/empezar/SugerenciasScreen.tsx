/**
 * Pantalla 06 · Extractos → sugerencias. La detección NUNCA crea sola: el
 * usuario confirma o descarta cada propuesta. Confirmar recurrente crea el
 * compromiso por su servicio canónico + refuerza learning rules. Completar
 * préstamo/nómina abre su wizard pre-rellenado (bloques C6).
 */
...
const SugerenciasScreen: React.FC = () => {
  const navigate = useNavigate();
  const { state, setBloque, refresh } = useOnboarding();
  ...
  const onConfirmar = async (sug: Sugerencia) => {
    ...
    await setBloque('finanzas', 'parcial', 'Sugerencias revisándose');
    void refresh();
  };
  ...
  return (
    <>
      ...
      <div className={styles.main}>
        ...
        <h1 className={styles.h1}>Esto es lo que Atlas ha encontrado</h1>
        <p className={styles.sub}>
          De los extractos de tus cuentas. Atlas nunca crea nada solo · tú confirmas o descartas cada sugerencia.
        </p>
        ...
        <div className={styles.sugSummary}>
          ...
          <div className={styles.kpi}>
            <div className={styles.kpiLabel}>Tiempo que te ahorra</div>
            <div className={`${styles.kpiVal} ${styles.mono}`}>~{(sugerencias.length + confirmadas) * 5} min</div>
            <div className={styles.kpiSub}>vs alta manual de todo</div>
          </div>
        </div>
        ...
        {cargando ? (
          ...
          <div className={styles.sugEmptyNote}>
            No hay sugerencias por ahora · sube extractos de tus cuentas para que Atlas deduzca tus recurrentes,
            préstamos y nómina.
            <button
              type="button"
            ...
            >
              <Icons.Upload size={14} strokeWidth={2} /> Subir extractos
            </button>
          </div>
        ) : (
          <>
