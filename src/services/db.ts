...
export const DB_VERSION = 96; // V96 (TAREA CC · E3.1b · un solo sitio para «quién cobra» · sep 2026): bump físico. Se ELIMINA el store `catalogoProveedores`, que nació en V95 y duró un commit — 47 stores otra vez. Motivo (Jose): dejaba `familia`/`subtipo` en DOS sitios para la misma pregunta sin dar nada que `proveedores` no pudiera dar, y `proveedores` ya está indexado por NIF, que es justo la clave del catálogo. `Proveedor` gana `ambito`, `alias[]`, `confirmaciones` y `origen` ('cliente' | 'nacional') OPCIONALES: ahí viven ahora tanto el NIF del fontanero del cliente como lo que haya confirmado sobre Iberdrola, y `origen: 'nacional'` marca lo ÚNICO compartible el día que esto viaje a un servidor (solo se pone sobre un CIF de EMPRESA: un DNI no sale del navegador de su dueño). Las 308 entidades del catálogo nacional NO viven en la base: viajan en el código (`services/catalogoNacional/catalogo-nacional-proveedores.json`) para que un cliente nuevo las tenga el día cero sin importar nada. `deleteObjectStore` bajo guard `contains()` en applyUpgradeA · sin migración de datos (el store nunca llegó a escribirse: su único escritor se conectó en el mismo commit que lo retira). Regla A: sin migración · sin post-open. 48 stores en v95 − 1. // V95 (TAREA CC · E3.1 · §7.3 · el catálogo nacional · sep 2026): bump físico. Nació el store `catalogoProveedores` (retirado en V96) y `Proveedor` ganó `familia`/`subtipo` OPCIONALES: hasta ahí solo tenía `tipos` AEAT (mejora/reparacion/gestion/servicios), que dice a qué casilla va pero no qué es. Regla A: sin migración de datos. // V94 (TAREA CC · E2.4.1c · un solo catálogo · sep 2026): bump físico. `compromisosRecurrentes` retira los índices `tipo` y `categoria` (los campos desaparecen: árbol B → familia/subtipo del catálogo único) y gana `familia`; `movementLearningRules` retira `categoria` y gana `familia`. `gastosInmueble.categoria` (árbol E) desaparece sin índice · `casillaAEAT` pasa a opcional (la pone la lente fiscal, no el catálogo). Regla A: sin migración de datos. 47 stores igual que v93. // V93 (TAREA CC · E2.4.1b · naturaleza · sep 2026): bump físico. `treasuryEvents` retira el índice `type` (el campo desaparece: `income|expense|financing` → naturaleza del catálogo único) y gana el índice `naturaleza`. `Movement.type` (Ingreso/Gasto/Transferencia/Ajuste) desaparece también, sin índice. Regla A: sin migración de datos (los registros con `type` viejo se recargan de cero). 47 stores igual que v92. // V92 (TAREA CC · E1.6 · limpieza tras el corte · sep 2026): bump físico SOLO para retirar el índice `duplicate-key` del store `movements` (accountId·date·amount·description). Nació en H8 «para detectar duplicados» y nunca tuvo un lector: el import deduplica por `hashMovement` en memoria (`bankStatementOrchestrator.huellasExistentes`). `deleteIndex` bajo guard `indexNames.contains()` en applyUpgradeA (gate `oldVersion < 92`) · sin migración de datos · sin post-open · 47 stores igual que v91. Regla B: retirado con grep de alcance a cero en src/ (solo el test de estructura lo nombraba). // V91 (TAREA CC · E1.1 · store `lineasExtracto` · sep 2026): bump físico. Nuevo store `lineasExtracto` (keyPath id autoIncrement · índices `importBatchId`, `accountId`, `hashLinea`, `estado`) — 47 stores. La línea del extracto deja de morir al insertar el `Movement`: el orquestador la persiste ADEMÁS de crear el movimiento (copia fiel del banco: `conceptoLiteral` exacto, importe con signo, fechas, referencia, saldo) con `movementIds` (plural) enlazando lo que engendró, y deja rastro de lo que NO generó movimiento (`descarte`: duplicada · sin_fecha · sin_importe), que antes se perdía en silencio. ADITIVO: ningún lector todavía, cero cambio de comportamiento. Store nace vía guard `contains()` en applyUpgradeA · sin migración de datos · sin post-open. Es el cimiento de E1.2-E1.5 (ver docs/VERIFICACION-E1-preflight-flujo-importacion-2026-09-03.md §6). // V90 (R1 · ALQUILERES · «poner en alquiler» · Jose 20 ago 2026): PRIMER bump físico desde v88. Nuevo store `explotacionAlquiler` (keyPath id autoIncrement · índice `inmuebleId`) — 46 stores. La explotación de un inmueble (modo completo/habitaciones/turístico · estado operativo/vacante/en_reforma · habitaciones con nombre+renta objetivo) DEJA de ser un atributo del inmueble y pasa a ser una entidad propia del dominio Alquileres que lo referencia (`inmuebleId`). Marcar un inmueble como alquilable = crear su `ExplotacionAlquiler`; desmarcarlo = borrarla. Lo NO marcado es uso propio (Personal, eje de imputación §9 ter). Store nace vía guard `contains()` en applyUpgradeA (aditivo · sin gate `oldVersion`). Migración POST-open idempotente (flag `migration_v90_explotacion_alquiler_v1`): siembra una `ExplotacionAlquiler` por cada inmueble con `modoExplotacion`/`alquilerPorHabitaciones` legacy poblado (mapeando piso_completo→completo, por_habitaciones/mixto→habitaciones, y usoTipo 'turistico'→turistico); los inmuebles `usoTipo` 'vivienda_habitual'/'disponible' NO se marcan (uso propio). Los campos legacy de `Property` quedan de solo-lectura hasta que no queden lectores. Ver docs/MODELO-OPERATIVO-tesoreria-v1.md §9 quinquies bis. // V89 (PLAN DE AMORTIZACIONES · VOCABULARIO §6 bis · quater): campos nuevos OPCIONALES, sin stores ni índices nuevos (los mismos que v88) — el upgrade es no-op, todas las creaciones siguen bajo su guard `contains()`. `Prestamo` gana `planDeAmortizaciones` (reglas + modo + cupo anual exento + gastos por operación + `generaPrevisiones`): lo que su dueño ha dicho que va a amortizar —«200 € todos los meses y la extra de junio»—, que NO es del contrato: el banco no sabe nada de esto, así que la forma vive aparte en `types/planDeAmortizaciones` y no en `types/prestamos`. Guardarlo NO es haberlo hecho: el cuadro no se toca, y lo que ocurra de verdad se sigue registrando el día que ocurra con `confirmLoanSettlement`. `TreasuryEvent.sourceType` gana `'amortizacion_anticipada'` para las previsiones que salen de ese plan — nacen `predicted` y solo si el plan lo pide, porque un plan a diez años son ciento veinte apuntes en «por confirmar» y eso lo decide quien la mira. Llevan clave propia por FECHA (`prestamo:<id>:amort:<fecha>`) y quedan fuera del atajo por mes del dedupe: con la clave por mes, planificar un extra en marzo daba por emitida la CUOTA de marzo y el recibo del banco desaparecía de la previsión justo por meter dinero. Sin migración de datos (campos opcionales · no-op en upgrade · sin post-open). // V88-FÍSICO (INVERSIONES V1 · Fase 1 · generalizar traspasos a fondos): PRIMER bump físico real desde v87. Añade el índice `activoId` al store `traspasosPlanPensiones` (createIndex en el upgrade callback · aditivo · se conservan `planId` y `fechaEjecucion`) para poder generalizar los traspasos fiscalmente neutros de planes a fondos siguiendo el patrón polimórfico de `valoracionesActivos` (activoId + tipoActivo). ⚠️ COLISIÓN DE NOMBRE «v88»: existía ya una migración POST-open de tarjetas etiquetada «V88» (flag `migration_v88_cuentas_tarjeta_v1`) que NUNCA subió la versión física (seguía en 87); esa post-open es INDEPENDIENTE de este bump y sigue corriendo por su flag. El backfill idempotente de este índice usa un flag propio y distinto (`migration_traspasos_activoId_idx`) precisamente para no confundirse con la de tarjetas. // V88-TARJETAS (VOCABULARIO §3 · la cuenta que era tarjeta se va): sin stores ni índices nuevos, así que en su momento la versión FÍSICA se quedó en 87 — es una migración POST-open idempotente (flag `migration_v88_cuentas_tarjeta_v1`) que corre DESPUÉS de V87, cuando la `Tarjeta` de esa cuenta ya existe. Decisión de Jose (4 ago 2026): las cuentas `TARJETA_CREDITO` se BORRAN con sus movimientos, sus lotes de importación y sus eventos, porque ese gasto ya lo cuenta el recibo que el banco cobra en la cuenta corriente y tenerlo por los dos lados era contar el mismo dinero dos veces. Se conserva la cuenta de la que algo vivo siga cobrando (una `Tarjeta` que se liquide ahí, otra cuenta, un préstamo, un contrato) y se sueltan los enlaces de los eventos que apuntaran a un movimiento borrado —un `executedMovementId` al vacío se lee como «ya se cobró»—. Deja recibo en `keyval['v88_recibo_de_borrado']`: borrar no se deshace. `Account.tipo` queda en CORRIENTE|EFECTIVO y se retiran `cardConfig`, `limiteCredito`, `deudaActual`, `bancoEmisor`, `diaCierre` y `diaPago`, que eran de la tarjeta y viven en `Tarjeta`. // V87 (VOCABULARIO §3 · la tarjeta deja de ser una cuenta): nuevo store `tarjetas` (keyPath id autoIncrement · índices `cuentaLiquidacionId`, `origen`, `activa`) — 45 stores. Una cuenta puede tener VARIAS tarjetas (lo normal son dos, débito y crédito) y eso no cabía en el `cardConfig` único de `accounts`; además una tarjeta de fuera (Carrefour, Cetelem) no puede quedar anclada a la cuenta donde hoy se domicilia, porque eso cambia. Migración POST-open ADITIVA e idempotente (flag `migration_v87_tarjetas_v1`): por cada cuenta `TARJETA_CREDITO` viva nace una `Tarjeta` de crédito con su cuenta de liquidación y su día de cargo; las cuentas BORRADAS se saltan. La cuenta NO se toca —sus movimientos son compras reales y borrarlos sería perder dinero de la vista—, y `Account.tipo` CONSERVA `TARJETA_CREDITO`: retirarlo exige antes un alta de tarjeta propia, porque hoy vive en el wizard de cuentas. Ver `docs/VOCABULARIO-dinero.md` §3. // V86 (VOCABULARIO §1 · tipos de cuenta): sin stores ni índices nuevos (44 igual que v85). `Account.tipo` retira `AHORRO` y `OTRA` — decisión de Jose: complicaban sin distinguir nada, porque una cuenta de ahorro se comporta exactamente igual que una corriente y "otra" no dice nada. Migración POST-open idempotente (flag `migration_v86_tipos_cuenta_v1`): las que existan pasan a `CORRIENTE`, conservando saldo, movimientos y todo lo demás. Ver `docs/VOCABULARIO-dinero.md` §1. // V85 (TAREA CC · TESORERÍA V6 · Bizum): campos nuevos OPCIONALES, sin stores ni índices nuevos (44 stores igual que v84). `MovementLearningRule` gana `aliasContraparte`/`contraparteCanonica`: la regla ya aprendía la CATEGORÍA de un movimiento, pero no de QUIÉN era. Cuando el usuario confirma "BIZUM DE ADNAN PARWEZ" contra la renta de "Adnan Parwez Khan" está enseñando que son la misma persona, y esa enseñanza se perdía; ahora se guarda junto a la regla que ya nace en ese mismo gesto (`confirmDecisions` → `feedLearningRule`). Ambos nombres se guardan TAL CUAL los escriben el banco y el contrato —se enseñan en la pantalla de reglas para que el usuario vea qué ha aprendido ATLAS y pueda borrarlo— y se normalizan (`claveDeNombre`) al LEERLOS, que es cuando hace falta comparar. Sólo se escriben cuando el parecido NO se deducía solo. Un `learnKey` guarda un alias (1→1): el mismo texto del banco confirmado después contra otra persona es una CORRECCIÓN y pisa al anterior. Sin migración de datos (campos opcionales · no-op en upgrade · sin post-open). // V84 (TAREA CC · TESORERÍA V6 · adenda 01 D1 + adenda 02 D1): campos nuevos OPCIONALES, sin stores ni índices nuevos (44 stores igual que v83). `TreasuryEvent` gana `descartado`/`descartadoAt`/`motivoDescarte`: descartar un previsto NO es borrarlo — el motor de previsiones tiene que saber que no va a ocurrir y el usuario tiene que poder deshacerlo (`deleteTreasuryEventCompletely()` queda reservado al borrado real). Un evento descartado no se muestra en Pendientes, no entra en los KPIs, no afecta al Cierre y no vuelve a proponerse; no toca ningún saldo (nunca ocurrió). `ImportBatch` gana `lineasIgnoradas[]` (`hashLinea` + `ignoradaAt`): el ignorado es de la SESIÓN DE IMPORTACIÓN, no del movimiento — como la línea ignorada no se materializa, no hay `Movement` contra el que deduplicar y el registro vive donde vive el fichero. Sin migración de datos (campos opcionales · no-op en upgrade · sin post-open). // V83 (TAREA CC · GASTOS-RECURRENTES · bump modelo): campos nuevos OPCIONALES en `CompromisoRecurrente` (sin stores/índices nuevos · 44 stores igual que v82): `cups`, `numeroContrato`, `margenGraciaDias`, `diaCargoIncierto`, `reparto[]` (RepartoInmueble embebido · reparto de un recibo entre varios inmuebles, sección 2.7); `ImporteEvento` gana `porTramos` y `porcentajeRenta` (sección 2.6); `MetodoPagoCompromiso` gana `bizum` (sección 2.8); `EstadoCompromiso` retira `pausado` y usa `activo|preparado|baja` (sección 2.3); `motivoBaja` pasa a enum `MotivoBaja` (sección 2.4). Sin migración de datos (campos opcionales · no-op en upgrade). Guard POST-open idempotente (flag `migration_v83_estado_preparado_v1`): 0 registros esperados (Jose verificó en copia 26/07); si aparece algún `pausado` NO se convierte, se registra y se avisa (PARA · decisión Jose). // V82 (TAREA CC · Bloque B · base amortizable por ejercicio): nuevo store `baseAmortizableEjercicio` (keyPath id autoIncrement · índice único `inmueble-ejercicio`). La base amortizable deja de ser el campo único `aeatAmortization.baseAmortizacion` (last-write-wins) y pasa a ser un dato por (inmueble·ejercicio) con procedencia (`origen`: xml/paralela/manual/heredada/conflicto). Migración POST-open idempotente (flag `migration_v82_base_amortizable_ejercicio_v1`): siembra cada año desde su casilla 0130 declarada (origen 'xml') y marca 'conflicto' los años cuyo 0130 difiere del campo único del inmueble (huella de una paralela). Años sin 0130 propio se heredan en LECTURA (servicio), sin invención de filas. 44 stores físicos en v82 (43 en v81 + 1). // V81 (TAREA CC · Bloque B · conectar presupuesto): campo único de conciliación. Se retira `Movement.estado_conciliacion` (tipo `ReconciliationStatus`) — era redundante con `unifiedStatus === 'conciliado'` (modelo unificado ATLAS HORIZON, escrito por todos los flujos). Migración de datos POST-open idempotente (flag keyval `migration_v81_conciliacion_unifiedStatus_v1`): para movimientos con el legacy `estado_conciliacion==='conciliado'` y `unifiedStatus` distinto, se fija `unifiedStatus='conciliado'` y se elimina la propiedad legacy · no se pierde ninguna conciliación histórica. Sin cambios de stores/índices (43 stores). // V80 (TAREA CC · Bloque A · conectar presupuesto): retirado el sistema de presupuesto persistido. Se eliminan los stores `presupuestos` y `presupuestoLineas` (modelo H9 `Presupuesto`/`PresupuestoLinea`) — nunca se usaron en producción (wizard inalcanzable, `/proyeccion/*` fuera del menú; Jose confirmó store vacío, sin dato que proteger). Migración destructiva controlada: `deleteObjectStore` bajo guard `contains()` en applyUpgradeA · sin migración de datos. 43 stores físicos en v80 (45 en v79 − 2). // V79 (onboarding día 0 · hueco 5.1): campo raíz nuevo `Property.estructuraCompra` (decisión Jose · NO anidado en `aeatAmortization` fiscal del pasado · §1) con 3 opcionales `aportacionPropia` (lo que el usuario puso), `importeFinanciado` (lo financiado) y `prestamoVinculadoId` (FK string a `Prestamo.id` uuid · NO number · decisión Jose D1). Migración no destructiva: campo opcional sin reescritura · properties existentes quedan con `estructuraCompra` undefined (no-op en el upgrade callback). 45 stores físicos en v79 (conteo canónico documentado sobre `interface AtlasHorizonDB`). // V78 (refactor modelo alquileres v3): nuevo store `botesAnualesSinIdentificar` (Camino 2 del wizard XML AEAT) + campo persistido `Property.modoExplotacion` (piso_completo/por_habitaciones/mixto) + campo `Contract.inquilino.cotitulares[]` (N NIFs en piso completo). Migración post-upgrade idempotente: deriva `modoExplotacion` del legacy `alquilerPorHabitaciones.activo`, inicializa `cotitulares=[]`, y elimina Contracts huérfanos `estadoContrato='sin_identificar'` + sus treasuryEvents en cascada (decisión Jose · reimportar limpio).
...
export interface AtlasHorizonDB extends DBSchema {
  ...
  explotacionAlquiler: { key: IDBValidKey; value: ExplotacionAlquiler; indexes: { 'inmuebleId': IDBValidKey } }; // V90 (R1): «poner en alquiler» como entidad propia · una por inmueble
  ...
  // catalogoProveedores: ELIMINADO en V96 (E3.1b · decisión de Jose) — nació en
  //   V95 y duró un commit. Dejaba `familia`/`subtipo` en DOS sitios para la
  //   misma pregunta («¿quién cobra y qué es?») sin dar nada que `proveedores`
  //   —que ya está indexado por NIF— no pudiera dar. Un proveedor es un
  //   proveedor. Lo aprendido vive ahora en `proveedores` con `alias`,
  //   `confirmaciones` y `origen`; el catálogo nacional de 308 entidades viaja
  //   en el código. 0 registros en producción (el store nunca llegó a escribirse).
  ...
  movements: { key: IDBValidKey; value: Movement; indexes: { 'accountId': IDBValidKey; 'date': IDBValidKey; 'importBatch': IDBValidKey; 'status': IDBValidKey } }; // H8: Bank movements
  ...
  lineasExtracto: { key: IDBValidKey; value: LineaExtractoPersistida; indexes: { 'accountId': IDBValidKey; 'estado': IDBValidKey; 'hashLinea': IDBValidKey; 'importBatchId': IDBValidKey } }; // V91 · E1.1: la línea del banco persistida · copia fiel · nadie la lee aún
  treasuryEvents: { key: IDBValidKey; value: TreasuryEvent; indexes: { 'accountId': IDBValidKey; 'ambito': IDBValidKey; 'año': IDBValidKey; 'certeza': IDBValidKey; 'generadoPor': IDBValidKey; 'inmuebleId': IDBValidKey; 'naturaleza': IDBValidKey; 'predictedDate': IDBValidKey; 'sourceId': IDBValidKey; 'sourceType': IDBValidKey; 'status': IDBValidKey } }; // H9: Treasury forecasting
  // treasuryRecommendations: ELIMINADO en V62 (sub-tarea 3) — derivable runtime · 0 registros
  // presupuestos + presupuestoLineas: ELIMINADOS en V80 (TAREA CC · Bloque A) — sistema de presupuesto persistido (modelo H9) retirado · wizard inalcanzable · 0 registros en producción
  ...
  tarjetas: { key: IDBValidKey; value: Tarjeta; indexes: { 'cuentaLiquidacionId': IDBValidKey; 'origen': IDBValidKey; 'activa': IDBValidKey } }; // V87 · VOCABULARIO §3: la tarjeta deja de ser una cuenta
  movementLearningRules: { key: IDBValidKey; value: MovementLearningRule; indexes: { 'ambito': IDBValidKey; 'appliedCount': IDBValidKey; 'familia': IDBValidKey; 'createdAt': IDBValidKey; 'learnKey': IDBValidKey } }; // V1.1: Learning rules for automatic classification
  ...
  traspasosPlanPensiones: { key: IDBValidKey; value: TraspasoPlanPensiones; indexes: { 'activoId': IDBValidKey; 'fechaEjecucion': IDBValidKey; 'planId': IDBValidKey } }; // V65: eventos traspaso fiscal neutro · V88 (INVERSIONES V1 Fase 1): índice `activoId` para consultar traspasos por activo polimórfico (plan o fondo)
  ...
  compromisosRecurrentes: { key: IDBValidKey; value: CompromisoRecurrente; indexes: { 'ambito': IDBValidKey; 'familia': IDBValidKey; 'cuentaCargo': IDBValidKey; 'estado': IDBValidKey; 'fechaInicio': IDBValidKey; 'inmuebleId': IDBValidKey; 'personalDataId': IDBValidKey } }; // V5.3: catálogo universal de compromisos (unifica opexRules + personal · G-01) · TAREA 9: bootstrap desde histórico vía `compromisoDetectionService` + creación idempotente vía `compromisoCreationService` · activa la vía A del `movementSuggestionService` cuando el store deja de estar vacío. Ver `docs/T9-cierre.md`.
  /**
  ...
  baseAmortizableEjercicio: { key: IDBValidKey; value: BaseAmortizableEjercicio; indexes: { 'inmuebleId': IDBValidKey; 'ejercicio': IDBValidKey; 'inmueble-ejercicio': IDBValidKey; 'origen': IDBValidKey } }; // V82: base amortizable por (inmueble·ejercicio) con procedencia · fuente de verdad = casilla 0130 declarada
